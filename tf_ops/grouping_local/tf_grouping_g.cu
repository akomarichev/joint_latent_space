...
__global__ void query_ball_point_adaptive_gpu(int b, int n, int m, const float *radius, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt) {
    int batch_index = blockIdx.x;
    xyz1 += n*3*batch_index;
    xyz2 += m*3*batch_index;
    idx += m*nsample*batch_index;
    pts_cnt += m*batch_index; // counting how many unique points selected in local region
    radius += m*batch_index;
    ...
    for (int j=index;j<m;j+=stride) {
        ...
        for (int k=0;k<n;++k) {
            ...
            // printf("Hello from block %d, radius %f\n", batch_index, radius[batch_index]);
            // if(radius[j] < 0.1 || radius[j] > 0.2)
            //     printf("------ Radius outside of the boundaries!!!!!!!!");
            if (d<radius[j] + 1e-10f) {
...
void queryBallPointAdaptiveLauncher(int b, int n, int m, const float *radius, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt) {
    query_ball_point_adaptive_gpu<<<b,256>>>(b,n,m,radius,nsample,xyz1,xyz2,idx,pts_cnt);
