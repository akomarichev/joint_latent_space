...
REGISTER_OP("QueryBallPointAdaptive")
    //.Attr("radius: float")
    .Attr("nsample: int")
    .Input("xyz1: float32")
    .Input("xyz2: float32")
...
void queryBallPointAdaptiveLauncher(int b, int n, int m, const float *radius, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt);
class QueryBallPointAdaptiveGpuOp : public OpKernel {
    public:
        explicit QueryBallPointAdaptiveGpuOp(OpKernelConstruction* context) : OpKernel(context) {
            ...
            OP_REQUIRES(context, nsample_ > 0, errors::InvalidArgument("QueryBallPointAdaptive expects positive nsample"));
        }

        void Compute(OpKernelContext* context) override {
            const Tensor& xyz1_tensor = context->input(0);
            OP_REQUIRES(context, xyz1_tensor.dims()==3 && xyz1_tensor.shape().dim_size(2)==3, errors::InvalidArgument("QueryBallPointAdaptive expects (batch_size, ndataset, 3) xyz1 shape."));
            int b = xyz1_tensor.shape().dim_size(0);
            int n = xyz1_tensor.shape().dim_size(1);

            const Tensor& xyz2_tensor = context->input(1);
            OP_REQUIRES(context, xyz2_tensor.dims()==3 && xyz2_tensor.shape().dim_size(2)==3, errors::InvalidArgument("QueryBallPointAdaptive expects (batch_size, npoint, 3) xyz2 shape."));
            int m = xyz2_tensor.shape().dim_size(1);

            const Tensor& radius_tensor = context->input(2);
            OP_REQUIRES(context, radius_tensor.dims()==2 && radius_tensor.shape().dim_size(0)==b, errors::InvalidArgument("QueryBallPointAdaptive expects RADIUS (batch_size, 1) shape."));
            ...
            queryBallPointAdaptiveLauncher(b,n,m,radius,nsample_,xyz1,xyz2,idx,pts_cnt);
        }
    private:
        //float radius_;
        int nsample_;
};
REGISTER_KERNEL_BUILDER(Name("QueryBallPointAdaptive").Device(DEVICE_GPU), QueryBallPointAdaptiveGpuOp);
